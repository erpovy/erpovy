                ...
                <x-card class="p-8 relative overflow-hidden group border-white/10 bg-white/5 backdrop-blur-2xl transition-all duration-500 hover:border-blue-500/30 hover:bg-white/10 hover:-translate-y-1 hover:shadow-2xl">
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div class="text-slate-400 text-xs font-black uppercase tracking-widest">Toplam Kontrol</div>
                        </div>
                ...
                <x-card class="p-8 relative overflow-hidden group border-white/10 bg-white/5 backdrop-blur-2xl transition-all duration-500 hover:border-green-500/30 hover:bg-white/10 hover:-translate-y-1 hover:shadow-2xl">
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div class="text-slate-400 text-xs font-black uppercase tracking-widest">Başarı Oranı</div>
                        </div>
                        <div class="text-5xl font-black text-white tracking-tight mb-2">%{{ $stats['pass_rate'] }}</div>
                ...
                <x-card class="p-8 relative overflow-hidden group border-white/10 bg-white/5 backdrop-blur-2xl transition-all duration-500 hover:border-red-500/30 hover:bg-white/10 hover:-translate-y-1 hover:shadow-2xl">
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div class="text-slate-400 text-xs font-black uppercase tracking-widest">Reddedilen</div>
                        </div>
                        <div class="text-5xl font-black text-white tracking-tight mb-2">{{ $stats['failed'] }}</div>
                        ...
                        <tbody class="divide-y divide-white/5">
                            @forelse($checks as $check)
                                <tr class="hover:bg-white/5 transition-colors group">
                                    <td class="px-8 py-6 font-bold text-white">{{ $check->reference_number }}</td>
                                    <td class="px-8 py-6">
                                    ...
                                    <td class="px-8 py-6">
                                        <div class="flex flex-col">
                                            <span class="text-white font-bold">{{ $check->product->name ?? 'Silinmiş Ürün' }}</span>
                                            <span class="text-xs text-slate-500 font-bold">{{ $check->product->code ?? '-' }}</span>
                                        </div>
                                    </td>
                                    <td class="px-8 py-6">
                                        @php
                                        ...
                                        @endphp
                                        <span class="text-slate-300 font-medium">{{ $types[$check->type] ?? $check->type }}</span>
                                    </td>
                                    <td class="px-8 py-6">
                                        <div class="flex flex-col">
                                            <span class="text-slate-300 font-bold">{{ number_format($check->checked_quantity, 0) }} Kontrol</span>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-8 py-16 text-center">
                                        ...
                                        <p class="text-slate-500 font-medium text-lg">Henüz kalite kontrol kaydı bulunmuyor.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
        ...
        <div x-show="openModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
            ...
            <div class="relative bg-[#1e1e2d] rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden border border-white/10 transform transition-all"
                ...
                <div class="px-8 py-6 border-b border-white/10 flex justify-between items-center bg-white/[0.02]">
                    <h3 class="text-xl font-black text-white">Yeni Kalite Kontrol Kaydı</h3>
                ...
                <form action="{{ route('manufacturing.quality.store') }}" method="POST" class="p-8 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                             <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Ürün</label>
                        ...
                        <!-- Check Date -->
                        <div>
                             <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Kontrol Tarihi</label>
                            <input type="date" name="check_date" value="{{ date('Y-m-d') }}" class="w-full rounded-xl border-white/10 bg-black/20 text-white focus:border-primary focus:ring-primary shadow-sm py-3 px-4" required>
                        </div>

                        <!-- Type -->
                        <div>
                             <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Kontrol Tipi</label>
                        ...
                        <!-- Quantities -->
                        <div>
                             <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Kontrol Edilen Miktar</label>
                            <input type="number" name="checked_quantity" step="0.01" min="0" class="w-full rounded-xl border-white/10 bg-black/20 text-white focus:border-primary focus:ring-primary shadow-sm py-3 px-4" required>
                        </div>
                        <div>
                             <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Reddedilen Miktar</label>
                            <input type="number" name="rejected_quantity" step="0.01" min="0" value="0" class="w-full rounded-xl border-white/10 bg-black/20 text-white focus:border-primary focus:ring-primary shadow-sm py-3 px-4">
                        </div>
                        
                         <!-- Status -->
                        <div class="md:col-span-2">
                              <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Karar</label>
                             <div class="flex gap-4">
                                 <label class="flex items-center gap-3 cursor-pointer p-4 rounded-xl border border-white/10 bg-white/[0.02] hover:bg-white/5 transition-colors flex-1 group">
                        ...
                        <!-- Notes -->
                        <div class="md:col-span-2">
                             <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Notlar / Hata Açıklaması</label>
                            <textarea name="notes" rows="3" class="w-full rounded-xl border-white/10 bg-black/20 text-white focus:border-primary focus:ring-primary shadow-sm py-3 px-4" placeholder="Varsa hata detayları veya notlar..."></textarea>
                        </div>
                    </div>
