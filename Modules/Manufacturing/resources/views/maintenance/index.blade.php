<x-app-layout>
    ...
    <div class="py-10" x-data="{ openModal: false, completeModal: false, selectedRecord: null }">
                ...
                <x-card class="p-8 relative overflow-hidden group border-white/10 bg-white/5 backdrop-blur-2xl transition-all duration-500 hover:border-blue-500/30 hover:bg-white/10 hover:-translate-y-1 hover:shadow-2xl">
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div class="text-slate-400 text-xs font-black uppercase tracking-widest">Planlı Bakım</div>
                        </div>
                        <div class="text-5xl font-black text-white tracking-tight mb-2">{{ $stats['planned'] }}</div>
                ...
                <x-card class="p-8 relative overflow-hidden group border-white/10 bg-white/5 backdrop-blur-2xl transition-all duration-500 hover:border-yellow-500/30 hover:bg-white/10 hover:-translate-y-1 hover:shadow-2xl">
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div class="text-slate-400 text-xs font-black uppercase tracking-widest">Devam Eden</div>
                        </div>
                        <div class="text-5xl font-black text-white tracking-tight mb-2">{{ $stats['in_process'] }}</div>
                ...
                <x-card class="p-8 relative overflow-hidden group border-white/10 bg-white/5 backdrop-blur-2xl transition-all duration-500 hover:border-green-500/30 hover:bg-white/10 hover:-translate-y-1 hover:shadow-2xl">
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div class="text-slate-400 text-xs font-black uppercase tracking-widest">Tamamlanan</div>
                        </div>
                        <div class="text-5xl font-black text-white tracking-tight mb-2">{{ $stats['completed'] }}</div>
                ...
                <x-card class="p-8 relative overflow-hidden group border-white/10 bg-white/5 backdrop-blur-2xl transition-all duration-500 hover:border-purple-500/30 hover:bg-white/10 hover:-translate-y-1 hover:shadow-2xl">
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div class="text-slate-400 text-xs font-black uppercase tracking-widest">Toplam Maliyet</div>
                        </div>
                        <div class="text-5xl font-black text-white tracking-tight mb-2 flex items-baseline gap-2">
                             {{ number_format($stats['total_cost'], 2) }} <span class="text-2xl text-slate-500">₺</span>
                        </div>
                                ...
                                <tr class="hover:bg-white/5 transition-colors group">
                                    ...
                                    <td class="px-8 py-6">
                                         <div class="flex flex-col">
                                            <span class="text-white font-bold">{{ $record->title }}</span>
                                            <span class="text-xs text-slate-500 font-bold mt-1 truncate max-w-[200px]">{{ $record->description }}</span>
                                        </div>
                                    </td>
                                    <td class="px-8 py-6">
                                        <div class="flex flex-col">
                                            <span class="text-white font-bold">{{ $record->workStation->name ?? 'Silinmiş İstasyon' }}</span>
                                            <span class="text-xs text-slate-500 font-bold">{{ $record->workStation->code ?? '-' }}</span>
                                        </div>
                                    </td>
                                    <td class="px-8 py-6">
                                        <div class="flex flex-col gap-1">
                                            <span class="text-xs font-black uppercase text-slate-400 tracking-wider">{{ $record->type }}</span>
                                             @php
                                                $priorityColors = [
                                                    'low' => 'text-slate-500',
                                    ...
                                    <td class="px-8 py-6">
                                        <div class="flex flex-col">
                                            <span class="text-white font-bold">{{ $record->start_date->format('d.m.Y') }}</span>
                                            <span class="text-xs text-slate-500 font-bold">{{ $record->start_date->format('H:i') }}</span>
                                        </div>
                                        @if($record->end_date)
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-8 py-16 text-center">
                                        ...
                                        <p class="text-slate-500 font-medium text-lg">Henüz bakım kaydı oluşturulmamış.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
        ...
        <div x-show="openModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
            ...
            <div class="relative bg-[#1e1e2d] rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden border border-white/10 transform transition-all"
                 x-show="openModal"
                ...
                <div class="px-8 py-6 border-b border-white/10 flex justify-between items-center bg-white/[0.02]">
                    <h3 class="text-xl font-black text-white">Yeni Bakım Planla</h3>
                ...
                <form action="{{ route('manufacturing.maintenance.store') }}" method="POST" class="p-8 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                             <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">İş İstasyonu / Makine</label>
                            <select name="work_station_id" class="w-full rounded-xl border-white/10 bg-black/20 text-white focus:border-primary focus:ring-primary shadow-sm py-3 px-4" required>
                        ...
                        <!-- Title -->
                        <div class="md:col-span-2">
                             <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Başlık</label>
                            <input type="text" name="title" class="w-full rounded-xl border-white/10 bg-black/20 text-white focus:border-primary focus:ring-primary shadow-sm py-3 px-4" placeholder="Örn: Haftalık Yağlama ve Kontrol" required>
                        </div>

                        <!-- Type -->
                        <div>
                             <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Bakım Tipi</label>
                        ...
                        <!-- Priority -->
                         <div>
                              <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Öncelik</label>
                            <select name="priority" class="w-full rounded-xl border-white/10 bg-black/20 text-white focus:border-primary focus:ring-primary shadow-sm py-3 px-4" required>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="datetime-local" name="start_date" class="w-full rounded-xl border-white/10 bg-black/20 text-white focus:border-primary focus:ring-primary shadow-sm py-3 px-4" required>
                        </div>

                        <!-- Description -->
                        <div class="md:col-span-2">
                             <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Açıklama</label>
                            <textarea name="description" rows="3" class="w-full rounded-xl border-white/10 bg-black/20 text-white focus:border-primary focus:ring-primary shadow-sm py-3 px-4"></textarea>
                        </div>
                    </div>
        ...
        <div x-show="completeModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
            ...
            <div class="relative bg-[#1e1e2d] rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-white/10 transform transition-all"
                ...
                <div class="px-8 py-6 border-b border-white/10 flex justify-between items-center bg-white/[0.02]">
                    <h3 class="text-xl font-black text-white">Bakımı Tamamla</h3>
                    <button @click="completeModal = false" class="text-slate-500 hover:text-white transition-colors">
                        <span class="material-symbols-outlined">close</span>
                    </button>
                </div>
                ...
                <form x-bind:action="'/manufacturing/maintenance/'+selectedRecord?.id" method="POST" class="p-8 space-y-6">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                         <!-- Cost -->
                        <div>
                             <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Maliyet (₺)</label>
                            <input type="number" name="cost" min="0" step="0.01" class="w-full rounded-xl border-white/10 bg-black/20 text-white focus:border-primary focus:ring-primary shadow-sm py-3 px-4" placeholder="0.00" required>
                        </div>
                        
                        <!-- Technician -->
                        <div>
                             <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Teknisyen / Yapan Kişi</label>
                            <input type="text" name="technician_name"  class="w-full rounded-xl border-white/10 bg-black/20 text-white focus:border-primary focus:ring-primary shadow-sm py-3 px-4" value="{{ auth()->user()->name }}">
                        </div>

                         <!-- Result Note -->
                        <div>
                             <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Sonuç Notu</label>
                            <textarea name="description" rows="2" class="w-full rounded-xl border-white/10 bg-black/20 text-white focus:border-primary focus:ring-primary shadow-sm py-3 px-4" placeholder="Yapılan işlemler..."></textarea>
                        </div>
                    </div>

                    <div class="pt-6 flex justify-end gap-4 border-t border-white/10">
                        <button type="button" @click="completeModal = false" class="px-6 py-3 rounded-xl text-slate-400 hover:text-white hover:bg-white/5 font-bold transition-colors">İptal</button>
