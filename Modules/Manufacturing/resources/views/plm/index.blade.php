    ...
    <div class="py-10" x-data="{ openModal: false, items: [{product_id: '', quantity: 1, unit: 'adet', wastage: 0}] }">
        <div class="container mx-auto max-w-[1920px] px-6 lg:px-12 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8">
                <!-- Total BOMs -->
                <x-card class="p-8 relative overflow-hidden group border-white/10 bg-white/5 backdrop-blur-2xl transition-all duration-500 hover:border-indigo-500/30 hover:bg-white/10 hover:-translate-y-1 hover:shadow-2xl">
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div class="text-slate-400 text-xs font-black uppercase tracking-widest">Toplam Reçete</div>
                        </div>
                ...
                <x-card class="p-8 relative overflow-hidden group border-white/10 bg-white/5 backdrop-blur-2xl transition-all duration-500 hover:border-green-500/30 hover:bg-white/10 hover:-translate-y-1 hover:shadow-2xl">
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div class="text-slate-400 text-xs font-black uppercase tracking-widest">Aktif Reçete</div>
                        </div>
                        ...
                        <tbody class="divide-y divide-white/5">
                            @forelse($boms as $bom)
                                <tr class="hover:bg-white/5 transition-colors group">
                                    <td class="px-8 py-6 font-bold text-white">{{ $bom->code }}</td>
                                    <td class="px-8 py-6 text-slate-300">{{ $bom->name ?? '-' }}</td>
                                    <td class="px-8 py-6">
                                        <div class="flex flex-col">
                                            <span class="text-white font-bold">{{ $bom->product->name ?? 'Silinmiş Ürün' }}</span>
                                            <span class="text-xs text-slate-500 font-bold">{{ $bom->product->code ?? '-' }}</span>
                                        </div>
                                    </td>
                                    <td class="px-8 py-6">
                                        <span class="px-2 py-1 rounded bg-white/10 text-xs font-mono text-slate-300 border border-white/5">
                                            {{ $bom->version }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-8 py-6 text-right">
                                        <button class="p-2 rounded-lg bg-white/5 text-slate-400 hover:text-white hover:bg-white/10 transition-colors">
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-8 py-16 text-center">
                                        ...
                                        <p class="text-slate-500 font-medium text-lg">Henüz ürün reçetesi oluşturulmamış.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
        ...
        <div x-show="openModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
            ...
            <div class="relative bg-[#1e1e2d] rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto border border-white/10 transform transition-all"
                 x-show="openModal"
                ...
                <div class="px-8 py-6 border-b border-white/10 flex justify-between items-center bg-white/[0.02] sticky top-0 z-10 backdrop-blur-md">
                    <h3 class="text-xl font-black text-white">Yeni Ürün Reçetesi (BOM)</h3>
                ...
                <form action="{{ route('manufacturing.plm.store') }}" method="POST" class="p-8 space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                         <div>
                            <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Ana Ürün</label>
                            <select name="product_id" class="w-full rounded-xl border-white/10 bg-black/20 text-white focus:border-primary focus:ring-primary shadow-sm py-3 px-4" required>
                            ...
                            <p class="text-xs text-slate-500 mt-2">Hangi ürün için reçete oluşturuyorsunuz?</p>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Reçete Adı</label>
                            <input type="text" name="name" class="w-full rounded-xl border-white/10 bg-black/20 text-white focus:border-primary focus:ring-primary shadow-sm py-3 px-4" placeholder="Örn: Standart Üretim Reçetesi">
                        </div>
                    </div>
                    ...
                    <div class="border-t border-white/10 pt-8">
                        ...
                        <div class="space-y-4">
                            <template x-for="(item, index) in items" :key="index">
                                <div class="flex flex-col md:flex-row gap-4 p-5 bg-white/[0.03] rounded-2xl border border-white/5 relative group hover:bg-white/[0.05] transition-colors">
                                    <div class="flex-1">
                                        <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Bileşen</label>
                                        <select :name="'items['+index+'][product_id]'" x-model="item.product_id" class="w-full rounded-lg border-white/10 bg-black/20 text-sm text-white focus:border-primary focus:ring-primary h-10 px-3" required>
                                    ...
                                    <div class="w-32">
                                        <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Miktar</label>
                                        <input type="number" :name="'items['+index+'][quantity]'" x-model="item.quantity" step="0.0001" class="w-full rounded-lg border-white/10 bg-black/20 text-sm text-white focus:border-primary focus:ring-primary h-10 px-3" placeholder="0">
                                    </div>
                                    <div class="w-28">
                                        <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Birim</label>
                                        <select :name="'items['+index+'][unit]'" x-model="item.unit" class="w-full rounded-lg border-white/10 bg-black/20 text-sm text-white focus:border-primary focus:ring-primary h-10 px-3">
                                     ...
                                     <div class="w-28">
                                        <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Fire (%)</label>
                                        <input type="number" :name="'items['+index+'][wastage_percent]'" x-model="item.wastage" step="0.01" class="w-full rounded-lg border-white/10 bg-black/20 text-sm text-white focus:border-primary focus:ring-primary h-10 px-3" placeholder="0">
                                    </div>
                        ...
                        <div x-show="items.length === 0" class="text-center py-12 text-slate-500 border-2 border-dashed border-white/10 rounded-2xl bg-white/[0.02]">
