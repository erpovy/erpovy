<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('inventory.warehouses.create') }}" class="px-4 py-2 bg-primary-600 hover:bg-primary-500 text-white rounded-lg shadow-neon transition-all flex items-center gap-2 text-sm font-bold">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-white/5">
                            <thead class="bg-white/5">
                                <tr>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Kod</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Depo Adı</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Yönetici</th>
                                ...
                                @forelse($warehouses as $warehouse)
                                    <tr class="hover:bg-white/5 transition-colors group">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-white">{{ $warehouse->name }}</div>
                                            @if($warehouse->is_default)
                                                <span class="text-[10px] bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded border border-emerald-500/20 uppercase font-black">Varsayılan</span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-400">
                                            {{ $warehouse->manager->name ?? 'Atanmamış' }}
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end gap-2">
                                                ...
                                                <form action="{{ route('inventory.warehouses.destroy', $warehouse) }}" method="POST" class="inline" onsubmit="return confirm('Bu depoyu silmek istediğinizden emin misiniz?');">
                                                    @csrf
                                                    @method('DELETE')
