            ...
            <div class="relative flex items-center justify-between py-4">
                <div>
                    ...
                    <p class="text-gray-500 dark:text-gray-400 text-sm font-medium flex items-center gap-2">
                        ...
                        #{{ $shipment->tracking_number }} - Detay ve Durum Güncelleme
                    </p>
                </div>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto">
            <div class="bg-white dark:bg-[#0f172a]/40 backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-3xl shadow-glass overflow-hidden shadow-2xl shadow-blue-500/5">
                <form action="{{ route('logistics.shipments.update', $shipment) }}" method="POST" class="p-8 space-y-6">
                    @csrf
                    @method('PUT')
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <select name="contact_id" required
                                ...
                                @foreach($contacts as $contact)
                                    <option value="{{ $contact->id }}" {{ (old('contact_id', $shipment->contact_id) == $contact->id) ? 'selected' : '' }}>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="tracking_number" value="{{ old('tracking_number', $shipment->tracking_number) }}" required
                        ...
                        <!-- Status -->
                        <div class="space-y-2">
                            <label class="text-xs font-black uppercase tracking-wider text-gray-500 dark:text-gray-400">Güncel Durum *</label>
                            <select name="status" required
                                class="w-full bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl px-4 py-3 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/50 outline-none transition-all">
                                <option value="pending" {{ $shipment->status == 'pending' ? 'selected' : '' }}>Beklemede</option>
                                <option value="in_transit" {{ $shipment->status == 'in_transit' ? 'selected' : '' }}>Taşıma Halinde</option>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="origin" value="{{ old('origin', $shipment->origin) }}" required
                                class="w-full bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl px-4 py-3 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/50 outline-none transition-all">
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="destination" value="{{ old('destination', $shipment->destination) }}" required
                                class="w-full bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl px-4 py-3 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/50 outline-none transition-all">
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="number" step="0.01" name="weight_kg" value="{{ old('weight_kg', $shipment->weight_kg) }}"
                                class="w-full bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl px-4 py-3 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/50 outline-none transition-all">
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="date" name="estimated_delivery" value="{{ old('estimated_delivery', $shipment->estimated_delivery ? $shipment->estimated_delivery->format('Y-m-d') : '') }}"
                                class="w-full bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl px-4 py-3 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/50 outline-none transition-all">
                        </div>
                    </div>
