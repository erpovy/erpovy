                    ...
                    <form action="{{ route('sales.rentals.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
                        ...
                        <div class="w-full md:w-64 relative" x-data="{ 
                            ...
                            <div x-show="open" 
                                 ...
                                 class="absolute top-full left-0 w-full mt-2 bg-slate-900 shadow-2xl rounded-xl border border-white/10 overflow-hidden z-50">
                                <div class="py-1">
                                    <button type="button" @click="select('', 'Tüm Durumlar')" style="padding-left: 20px !important;" class="w-full text-left px-4 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white transition-colors border-b border-white/5">Tüm Durumlar</button>
                                    <button type="button" @click="select('active', 'Aktif')" style="padding-left: 20px !important;" class="w-full text-left px-4 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white transition-colors border-b border-white/5">Aktif</button>
                                    <button type="button" @click="select('completed', 'Tamamlandı')" style="padding-left: 20px !important;" class="w-full text-left px-4 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white transition-colors border-b border-white/5">Tamamlandı</button>
                                    <button type="button" @click="select('overdue', 'Gecikti')" style="padding-left: 20px !important;" class="w-full text-left px-4 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white transition-colors border-b border-white/5">Gecikti</button>
                                    <button type="button" @click="select('cancelled', 'İptal')" style="padding-left: 20px !important;" class="w-full text-left px-4 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white transition-colors">İptal</button>
                                </div>
                            </div>
                        </div>
                    </form>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-white/[0.02] border-b border-white/5">
                                <th class="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Kiralama No / Ürün</th>
                                <th class="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Müşteri</th>
                                <th class="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider text-right">Günlük Fiyat</th>
                                <th class="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider text-center">Başlangıç</th>
                                <th class="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider text-center">Bitiş</th>
                                <th class="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider text-center">Durum</th>
                                <th class="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider text-right">İşlem</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-white/5">
                            ...
                            <tr class="hover:bg-white/5 transition-colors group">
                                <td class="p-4">
                                    ...
                                    <div class="text-[10px] text-slate-500 font-bold uppercase">{{ $rental->product->name ?? 'Genel Kiralama' }}</div>
                                </td>
                                <td class="p-4">
                                    <div class="text-sm font-medium text-slate-200">{{ $rental->contact->name ?? '---' }}</div>
                                </td>
                                ...
                                <td class="p-4 text-center">
                                    <div class="text-xs font-medium {{ $rental->end_date && $rental->end_date->isPast() && $rental->status == 'active' ? 'text-red-400 font-bold' : 'text-slate-300' }}">
                                ...
                                <td class="p-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('sales.rentals.show', $rental) }}" class="p-2 rounded-lg bg-white/5 text-slate-400 hover:bg-white/10 transition-all">
                            ...
                            @empty
                            <tr>
                                <td colspan="7" class="p-12 text-center text-slate-500 italic">Kiralama kaydı bulunamadı.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
