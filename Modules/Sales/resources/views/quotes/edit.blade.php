<x-app-layout>
            ...
            <div class="relative flex items-center justify-between py-2">
                <div>
                    ...
                    <p class="text-slate-400 text-sm font-medium flex items-center gap-2">
                        ...
                        #{{ $quote->quote_number }} - Teklif detaylarını güncelleyin
                    </p>
                </div>
                <div class="flex items-center gap-3">
                    <a href="{{ route('sales.quotes.index') }}" class="px-6 py-2 rounded-xl bg-white/5 text-white border border-white/10 font-black text-sm hover:bg-white/10 transition-all flex items-center gap-2">
                        İptal Et
                    </a>
                </div>
            </div>
    ...
    <div class="py-8">
        <div class="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <form action="{{ route('sales.quotes.update', $quote) }}" method="POST" x-data="quoteForm()">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <div x-data="{ 
                                        open: false, 
                                        search: '', 
                                        selectedId: '{{ $quote->contact_id }}', 
                                        selectedName: '{{ addslashes($quote->contact->name) }}',
                                        contacts: [
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="date" name="date" required value="{{ $quote->date->format('Y-m-d') }}" class="w-full bg-white/5 border border-white/10 rounded-2xl py-3 px-4 text-white focus:border-primary/50 focus:ring-0 transition-all">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Geçerlilik</label>
                                        <input type="date" name="expiry_date" value="{{ $quote->expiry_date ? $quote->expiry_date->format('Y-m-d') : '' }}" class="w-full bg-white/5 border border-white/10 rounded-2xl py-3 px-4 text-white focus:border-primary/50 focus:ring-0 transition-all">
                                    </div>
                                </div>
                            </div>
                        </x-card>
                    ...
                    <div class=" space-y-8">
                        <x-card class="p-8 border-white/10 bg-white/5 backdrop-blur-2xl">
                            ...
                            <a href="{{ route('sales.quotes.show', $quote) }}" class="w-full mt-4 flex items-center justify-center py-4 text-xs font-black text-slate-400 hover:text-slate-900 transition-colors uppercase tracking-widest">
                                İşlemi İptal Et
                            </a>
                        </x-card>
                        ...
                        <x-card class="p-8 border-white/10 bg-white/5 backdrop-blur-2xl">
                            ...
                            <textarea name="notes" rows="4" placeholder="Müşteriye özel notlar..." class="w-full bg-white/5 border border-white/10 rounded-2xl py-3 px-4 text-sm text-white focus:border-primary/50 focus:ring-0 transition-all font-medium">{{ $quote->notes }}</textarea>
