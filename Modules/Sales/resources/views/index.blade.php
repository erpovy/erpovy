                ...
                <div class="lg:col-span-1 space-y-6">
                    <x-card class="p-8 border-white/10 bg-white/5 backdrop-blur-2xl">
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            ...
                            <a href="{{ $action['route'] }}" class="group flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/10 hover:border-{{ $action['color'] }}-500/30 hover:bg-{{ $action['color'] }}-500/5 transition-all">
                                ...
                                <span class="font-black text-slate-300 group-hover:text-white uppercase tracking-wider text-sm">{{ $action['label'] }}</span>
                            </a>
                            @endforeach
                        </div>
                    </x-card>
                    ...
                    <x-card class="h-full p-8 border-white/10 bg-white/5 backdrop-blur-2xl flex flex-col">
                        <div class="flex items-center justify-between mb-8">
                            <div>
                                ...
                                <p class="text-xs text-slate-500 mt-1">Sistem üzerinden gerçekleşen son işlemler</p>
                            </div>
                            <a href="#" class="px-4 py-2 bg-white/5 rounded-xl border border-white/10 text-xs font-black text-slate-400 hover:text-white transition-colors uppercase tracking-widest">Tümünü Gör</a>
                        </div>

                        <div class="flex-1 space-y-4">
                            @forelse($recentSales as $sale)
                            <div class="flex items-center gap-6 p-4 rounded-2xl hover:bg-white/5 border border-transparent hover:border-white/10 transition-all group">
                                <div class="w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center text-slate-500 group-hover:bg-primary/10 group-hover:text-primary transition-all">
                                    <span class="material-symbols-outlined text-[28px]">receipt_long</span>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="text-sm font-black text-white group-hover:text-primary transition-colors truncate">
                                ...
                                <div class="text-right">
                                    <div class="text-lg font-black text-white">₺{{ number_format($sale->total_amount, 0, ',', '.') }}</div>
