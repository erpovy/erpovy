        ...
        <div class="container mx-auto max-w-[1920px] px-6 lg:px-8 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <x-card class="p-6 border-white/10 bg-white/5 backdrop-blur-2xl">
                    ...
                    <h3 class="text-sm font-medium text-slate-400 mb-1">Toplam Gelir</h3>
                ...
                <x-card class="p-6 border-white/10 bg-white/5 backdrop-blur-2xl">
                    ...
                    <h3 class="text-sm font-medium text-slate-400 mb-1">Toplam Gider</h3>
                ...
                <x-card class="p-6 border-white/10 bg-white/5 backdrop-blur-2xl">
                    ...
                    <h3 class="text-sm font-medium text-slate-400 mb-1">Net {{ $net_income >= 0 ? 'Kar' : 'Zarar' }}</h3>
                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="border-b border-white/10">
                                    ...
                                    <th class="text-right py-3 px-4 text-sm font-semibold text-slate-400">Tutar</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($revenues as $revenue)
                                    <tr class="border-b border-white/5 hover:bg-white/5 transition-colors">
                                        <td class="py-3 px-4 text-sm text-slate-300 font-mono">{{ $revenue['code'] }}</td>
                                        <td class="py-3 px-4 text-sm text-white">{{ $revenue['name'] }}</td>
                                ...
                                <tr class="bg-green-500/10 border-t-2 border-green-500/30">
                                    <td colspan="2" class="py-3 px-4 text-sm font-bold text-white">TOPLAM GELİR</td>
                        ...
                        <table class="w-full">
                            ...
                            <tbody>
                                @foreach($expenses as $expense)
                                    <tr class="border-b border-white/5 hover:bg-white/5 transition-colors">
                                        <td class="py-3 px-4 text-sm text-slate-300 font-mono">{{ $expense['code'] }}</td>
                                        <td class="py-3 px-4 text-sm text-white">{{ $expense['name'] }}</td>
                                ...
                                <tr class="bg-red-500/10 border-t-2 border-red-500/30">
                                    <td colspan="2" class="py-3 px-4 text-sm font-bold text-white">TOPLAM GİDER</td>
            ...
            <!-- Net Kar/Zarar Özeti -->
            <x-card class="p-6 border-white/10 bg-gradient-to-br from-{{ $net_income >= 0 ? 'blue' : 'orange' }}-500/10 to-{{ $net_income >= 0 ? 'cyan' : 'red' }}-500/10 backdrop-blur-2xl border-{{ $net_income >= 0 ? 'blue' : 'orange' }}-500/30">
                <div class="text-center">
                    <h3 class="text-lg font-semibold text-slate-300 mb-2">Dönem Sonu Net {{ $net_income >= 0 ? 'Kar' : 'Zarar' }}</h3>
                    <p class="text-5xl font-black text-{{ $net_income >= 0 ? 'blue' : 'orange' }}-400 mb-4">{{ number_format(abs($net_income), 2) }}₺</p>
                    <p class="text-sm text-slate-400">{{ $start_date }} - {{ $end_date }} dönemi</p>
                </div>
            </x-card>
