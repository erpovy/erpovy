<x-app-layout>
    <x-slot name="header">
        Faturayı Düzenle: #{{ $invoice->invoice_number }}
    </x-slot>

    <div class="max-w-7xl mx-auto">
        <form action="{{ route('accounting.invoices.update', $invoice->id) }}" method="POST"
              x-data="invoiceForm({{ json_encode($invoice->items) }}, '{{ $invoice->issue_date->format('Y-m-d') }}', '{{ $invoice->due_date ? $invoice->due_date->format('Y-m-d') : '' }}')">
            @csrf
            @method('PUT')
            ...
            <x-card class="mb-6 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-400 mb-1">Vade Tarihi</label>
                            ...
                            <template x-for="(item, index) in items" :key="index">
                                <tr>
                                    <td class="py-2 pr-2">
                                        ...
                                        <!-- Add hidden ID field for update logic if needed, usually we replace items or update by ID -->
                                        <input type="hidden" :name="'items['+index+'][id]'" x-model="item.id" x-if="item.id">
                                    </td>
                                    <td class="py-2">
            ...
            <div class="flex justify-end gap-4">
                <a href="{{ route('accounting.invoices.show', $invoice->id) }}" class="px-6 py-3 rounded-xl border border-white/10 text-slate-300 hover:bg-white/5 transition-colors font-medium">
                    İptal
                </a>
    ...
    <script>
        function invoiceForm(existingItems = [], initialIssueDate = '', initialDueDate = '') {
            let initialItems = [];
            
            if (existingItems && existingItems.length > 0) {
                // Map existing items to Alpine data structure
