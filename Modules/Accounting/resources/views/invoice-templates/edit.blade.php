<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white leading-tight">
            {{ __('Fatura Şablonunu Düzenle') }}
        </h2>
    </x-slot>

    <div class="max-w-7xl mx-auto py-6">
        <form action="{{ route('accounting.invoice-templates.update', $template->id) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div class="p-6 bg-slate-800 rounded-lg border border-slate-700">
                        <div class="mb-4">
                            ...
                            <input type="text" name="name" id="name" value="{{ $template->name }}" required class="w-full bg-slate-900 text-slate-200 border-slate-700 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                        </div>

                        <div class="mt-4">
                            <label class="block text-sm font-medium text-slate-300 mb-2">HTML İçeriği</label>
                            <textarea name="html_content" id="html_content" rows="20" class="w-full bg-slate-900 text-slate-200 border-slate-700 rounded-lg font-mono text-sm p-4 focus:ring-indigo-500 focus:border-indigo-500">{{ old('html_content', $template->html_content) }}</textarea>
                ...
                <div class="space-y-6">
                    <div class="p-6 bg-slate-800 rounded-lg border border-slate-700">
                        <div class="flex items-center gap-3 mb-6">
                            <input type="checkbox" name="is_default" id="is_default" value="1" {{ $template->is_default ? 'checked' : '' }} class="rounded bg-slate-800 border-slate-600 text-indigo-600 focus:ring-indigo-500">
    ...
    <script>
        ...
        function closePreview() {
            ...
            document.getElementById('previewModal').classList.remove('block');
        }
    </script>
</x-app-layout>
