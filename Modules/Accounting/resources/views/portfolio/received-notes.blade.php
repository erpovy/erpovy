    ...
    <x-slot name="header">
        <div class="relative overflow-hidden group">
            ...
            <div class="relative flex items-center justify-between py-2">
                ...
                <a href="{{ route('accounting.promissory-notes.create', ['type' => 'received']) }}" class="px-6 py-3 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white rounded-xl font-semibold shadow-lg shadow-blue-500/30 hover:shadow-blue-500/50 transition-all flex items-center gap-2">
            ...
            <x-card class="border-white/10 bg-white/5 backdrop-blur-2xl overflow-hidden">
                        ...
                        <table class="w-full">
                            <thead class="bg-white/5 border-b border-white/10">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Borçlu</th>
                                ...
                                @foreach($notes as $note)
                                    <tr class="hover:bg-white/5 transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <a href="{{ route('accounting.promissory-notes.show', $note) }}" class="text-sm font-medium text-blue-400 hover:text-blue-300">
                                                {{ $note->note_number }}
                                            </a>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-slate-300">{{ $note->contact?->name ?? $note->drawer }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('accounting.promissory-notes.show', $note) }}" class="text-blue-400 hover:text-blue-300">
                                                Görüntüle
                                            </a>
                                        </td>
                                    </tr>
                ...
                @else
                    <div class="p-12 text-center">
                        <span class="material-symbols-outlined text-6xl text-slate-600 mb-4">description</span>
                        <h3 class="text-xl font-semibold text-slate-400 mb-2">Alınan senet bulunmuyor</h3>
                        <p class="text-slate-500 mb-6">Portföyünüzde henüz alınan senet bulunmamaktadır.</p>
                        <a href="{{ route('accounting.promissory-notes.create', ['type' => 'received']) }}" class="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-semibold transition-all">
                            <span class="material-symbols-outlined">add</span>
                            Yeni Senet Ekle
                        </a>
                    </div>
