<x-app-layout>
    <x-slot name="header">
        <div class="relative overflow-hidden group">
            <div class="absolute inset-0 bg-gradient-to-r from-red-500/5 via-orange-500/5 to-red-500/5 animate-pulse"></div>
            ...
            <div class="relative flex items-center justify-between py-2">
                ...
                <a href="{{ route('accounting.cheques.create', ['type' => 'issued']) }}" class="px-6 py-3 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-500 hover:to-orange-500 text-white rounded-xl font-semibold shadow-lg shadow-red-500/30 hover:shadow-red-500/50 transition-all flex items-center gap-2">
            ...
            <x-card class="border-white/10 bg-white/5 backdrop-blur-2xl overflow-hidden">
                        ...
                        <table class="w-full">
                            <thead class="bg-white/5 border-b border-white/10">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-300 uppercase tracking-wider">Keşideci (Biz)</th>
                                ...
                                @foreach($cheques as $cheque)
                                    <tr class="hover:bg-white/5 transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <a href="{{ route('accounting.cheques.show', $cheque) }}" class="text-sm font-medium text-red-400 hover:text-red-300">
                                                {{ $cheque->cheque_number }}
                                            </a>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-slate-300">{{ $cheque->bank_name }}</td>
                                        <td class="px-6 py-4 text-sm text-slate-300">{{ $cheque->drawer }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('accounting.cheques.show', $cheque) }}" class="text-red-400 hover:text-red-300">
                                                Görüntüle
                                            </a>
                                        </td>
                                    </tr>
                ...
                @else
                    <div class="p-12 text-center">
                        ...
                        <h3 class="text-xl font-semibold text-slate-400 mb-2">Verilen çek bulunmuyor</h3>
                        <p class="text-slate-500 mb-6">Portföyünüzde henüz verilen çek bulunmamaktadır.</p>
                        <a href="{{ route('accounting.cheques.create', ['type' => 'issued']) }}" class="inline-flex items-center gap-2 px-6 py-3 bg-red-600 hover:bg-red-500 text-white rounded-xl font-semibold transition-all">
                            <span class="material-symbols-outlined">add</span>
                            Yeni Çek Ekle
                        </a>
                    </div>
