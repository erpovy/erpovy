    ...
    <div class="py-8">
        <div class="container mx-auto max-w-[1920px] px-6 lg:px-8 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <!-- Alınan Çekler -->
                <x-card class="p-6 border-gray-200 dark:border-white/10 bg-white/80 dark:bg-white/5 backdrop-blur-2xl hover:bg-gray-100 dark:hover:bg-white/10 transition-all group cursor-pointer" onclick="window.location='{{ route('accounting.portfolio.received-cheques') }}'">
                    <div class="flex items-start justify-between mb-4">
                        ...
                        <span class="text-xs text-gray-600 dark:text-slate-500">{{ $stats['received_cheques_count'] }} adet</span>
                    </div>
                    <h3 class="text-sm font-medium text-gray-600 dark:text-slate-400 mb-1">Alınan Çekler</h3>
                    <p class="text-2xl font-black text-green-400">{{ number_format($stats['received_cheques_amount'], 2) }}₺</p>
                </x-card>

                <!-- Verilen Çekler -->
                <x-card class="p-6 border-gray-200 dark:border-white/10 bg-white/80 dark:bg-white/5 backdrop-blur-2xl hover:bg-gray-100 dark:hover:bg-white/10 transition-all group cursor-pointer" onclick="window.location='{{ route('accounting.portfolio.issued-cheques') }}'">
                    <div class="flex items-start justify-between mb-4">
                        ...
                        <span class="text-xs text-gray-600 dark:text-slate-500">{{ $stats['issued_cheques_count'] }} adet</span>
                    </div>
                    <h3 class="text-sm font-medium text-gray-600 dark:text-slate-400 mb-1">Verilen Çekler</h3>
                    <p class="text-2xl font-black text-red-400">{{ number_format($stats['issued_cheques_amount'], 2) }}₺</p>
                </x-card>

                <!-- Alınan Senetler -->
                <x-card class="p-6 border-gray-200 dark:border-white/10 bg-white/80 dark:bg-white/5 backdrop-blur-2xl hover:bg-gray-100 dark:hover:bg-white/10 transition-all group cursor-pointer" onclick="window.location='{{ route('accounting.portfolio.received-notes') }}'">
                    <div class="flex items-start justify-between mb-4">
                        ...
                        <span class="text-xs text-gray-600 dark:text-slate-500">{{ $stats['received_notes_count'] }} adet</span>
                    </div>
                    <h3 class="text-sm font-medium text-gray-600 dark:text-slate-400 mb-1">Alınan Senetler</h3>
                    <p class="text-2xl font-black text-blue-400">{{ number_format($stats['received_notes_amount'], 2) }}₺</p>
                </x-card>

                <!-- Verilen Senetler -->
                <x-card class="p-6 border-gray-200 dark:border-white/10 bg-white/80 dark:bg-white/5 backdrop-blur-2xl hover:bg-gray-100 dark:hover:bg-white/10 transition-all group cursor-pointer" onclick="window.location='{{ route('accounting.portfolio.issued-notes') }}'">
                    <div class="flex items-start justify-between mb-4">
                        ...
                        <span class="text-xs text-gray-600 dark:text-slate-500">{{ $stats['issued_notes_count'] }} adet</span>
                    </div>
                    <h3 class="text-sm font-medium text-gray-600 dark:text-slate-400 mb-1">Verilen Senetler</h3>
                ...
                <a href="{{ route('accounting.cheques.create') }}" class="block">
                    <x-card class="p-6 border-gray-200 dark:border-white/10 bg-white/80 dark:bg-white/5 backdrop-blur-2xl hover:bg-gray-100 dark:hover:bg-white/10 transition-all group">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h3 class="text-gray-900 dark:text-white font-semibold">Yeni Çek</h3>
                                <p class="text-sm text-gray-600 dark:text-slate-400">Çek ekle</p>
                            </div>
                        </div>
                    </x-card>
                </a>
                ...
                <a href="{{ route('accounting.promissory-notes.create') }}" class="block">
                    <x-card class="p-6 border-gray-200 dark:border-white/10 bg-white/80 dark:bg-white/5 backdrop-blur-2xl hover:bg-gray-100 dark:hover:bg-white/10 transition-all group">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h3 class="text-gray-900 dark:text-white font-semibold">Yeni Senet</h3>
                                <p class="text-sm text-gray-600 dark:text-slate-400">Senet ekle</p>
                            </div>
                        </div>
                    </x-card>
                </a>
                ...
                <a href="{{ route('accounting.portfolio.upcoming') }}" class="block">
                    <x-card class="p-6 border-gray-200 dark:border-white/10 bg-white/80 dark:bg-white/5 backdrop-blur-2xl hover:bg-gray-100 dark:hover:bg-white/10 transition-all group">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h3 class="text-gray-900 dark:text-white font-semibold">Yaklaşan Vadeler</h3>
                                <p class="text-sm text-gray-600 dark:text-slate-400">Vade takvimi</p>
                            </div>
                        </div>
                    </x-card>
                </a>
            ...
            @if($overdueCheques->count() > 0 || $overdueNotes->count() > 0)
                ...
                <x-card class="p-6 border-red-500/30 bg-gradient-to-br from-red-500/10 to-orange-500/10 backdrop-blur-2xl">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1">
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-2">⚠️ Vadesi Geçmiş Çek/Senetler</h3>
                            <p class="text-sm text-gray-700 dark:text-slate-300 mb-4">Aşağıdaki çek ve senetlerin vadesi geçmiştir. Lütfen gerekli işlemleri yapınız.</p>
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="space-y-2">
                                        @foreach($overdueCheques as $cheque)
                                            <div class="flex items-center justify-between p-3 rounded-lg bg-gray-100 dark:bg-white/5">
                                                <div>
                                                    <p class="text-sm text-gray-900 dark:text-white font-medium">{{ $cheque->cheque_number }} - {{ $cheque->bank_name }}</p>
                                                    <p class="text-xs text-gray-600 dark:text-slate-400">{{ $cheque->contact?->name ?? $cheque->drawer }}</p>
                                                </div>
                                                <div class="text-right">
                                                    <p class="text-sm font-bold text-red-400">{{ number_format($cheque->amount, 2) }}₺</p>
                                                    <p class="text-xs text-gray-600 dark:text-slate-400">{{ $cheque->due_date->format('d.m.Y') }}</p>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                                ...
                                <div>
                                    ...
                                    <div class="space-y-2">
                                        @foreach($overdueNotes as $note)
                                            <div class="flex items-center justify-between p-3 rounded-lg bg-gray-100 dark:bg-white/5">
                                                <div>
                                                    <p class="text-sm text-gray-900 dark:text-white font-medium">{{ $note->note_number }}</p>
                                                    <p class="text-xs text-gray-600 dark:text-slate-400">{{ $note->contact?->name ?? $note->drawer }}</p>
                                                </div>
                                                <div class="text-right">
                                                    <p class="text-sm font-bold text-red-400">{{ number_format($note->amount, 2) }}₺</p>
                                                    <p class="text-xs text-gray-600 dark:text-slate-400">{{ $note->due_date->format('d.m.Y') }}</p>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                ...
                <x-card class="p-6 border-gray-200 dark:border-white/10 bg-white/80 dark:bg-white/5 backdrop-blur-2xl">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        @if($upcomingCheques->count() > 0)
                            <div>
                                <h4 class="text-sm font-semibold text-gray-700 dark:text-slate-300 mb-3">Çekler</h4>
                                <div class="space-y-2">
                                    @foreach($upcomingCheques->take(5) as $cheque)
                                        <div class="flex items-center justify-between p-3 rounded-lg bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 transition-all">
                                            <div class="flex-1">
                                                <p class="text-sm text-gray-900 dark:text-white font-medium">{{ $cheque->cheque_number }}</p>
                                                <p class="text-xs text-gray-600 dark:text-slate-400">{{ $cheque->contact?->name ?? $cheque->drawer }}</p>
                                            </div>
                                            <div class="text-right">
                        ...
                        @if($upcomingNotes->count() > 0)
                            <div>
                                <h4 class="text-sm font-semibold text-gray-700 dark:text-slate-300 mb-3">Senetler</h4>
                                <div class="space-y-2">
