<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <x-card>
                <form action="{{ route('hr.roles.store') }}" method="POST" class="space-y-6">
                    @csrf
                         ...
                         <div>
                            ...
                            <select name="department_id" class="custom-input text-white !bg-[#0f172a]">
                                ...
                                @foreach($departments as $department)
                                    <option value="{{ $department->id }}" {{ old('department_id') == $department->id ? 'selected' : '' }} class="bg-[#0f172a] text-white">
                                        {{ $department->name }}
                                    </option>
                                @endforeach
                            </select>
